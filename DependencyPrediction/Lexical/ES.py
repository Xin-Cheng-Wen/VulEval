...
def edit_similarity(
    ...
    if isinstance(code1, str):
        ...
        code1 = tokenizer.tokenize(code1, truncation=True, max_length= 2048)
    elif isinstance(code1, list):
        pass

    if isinstance(code2, str):
        assert tokenizer, "tokenizer must be provided if input is string"
        code2 = tokenizer.tokenize(code2, truncation=True, max_length= 2048)
...
def jaccard_similarity(
    ...
    if isinstance(code1, str):
        assert tokenizer, "tokenizer must be provided if input is string"
        code1 = set(tokenizer.tokenize(code,truncation=True, max_length= 20481))
    elif isinstance(code1, list):
        code1 = set(code1)

    if isinstance(code2, str):
        assert tokenizer, "tokenizer must be provided if input is string"
        code2 = set(tokenizer.tokenize(code2,truncation=True, max_length= 2048))
    elif isinstance(code2, list):
        code2 = set(code2)
...
def lexical_rag(code, rag_querys, rag_name_querys, k):
...
def retrieve(
    ...
    if similarity == "cosine":
        ...
        def get_embedding(code):
            ...
            if isinstance(model, UniXcoder):
                # this is for unixcoder
                tokens_ids = model.tokenize([code], truncation=True, max_length=max_length, mode="<encoder-only>")
...
for random_seed in range(1):
    ...
    with open(json_file, "r") as function_file:
        ...
        for function_line in function_file:
            ...
            rag_index = lexical_rag(code, rag_querys, rag_name_querys, 1)
            ...
            rag_index = lexical_rag(code, rag_querys, rag_name_querys, 3)
            rag_name_list = []
            for i in range(len(rag_index)):
                rag_name_list.append(rag_name_querys[rag_index[i]])
            ...
            rag_index = lexical_rag(code, rag_querys, rag_name_querys, 5)
            rag_name_list = []
            for i in range(len(rag_index)):
                rag_name_list.append(rag_name_querys[rag_index[i]])
            ...
            rag_index = lexical_rag(code, rag_querys, rag_name_querys, 10)
            rag_name_list = []
            for i in range(len(rag_index)):
                rag_name_list.append(rag_name_querys[rag_index[i]])
